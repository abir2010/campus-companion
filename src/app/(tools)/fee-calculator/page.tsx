...
export default function FeeCalculatorPage() {
  ...
  const calculateFees = (e: FormEvent) => {
    e.preventDefault();
    const dcRegularTuition = inputs.dcRegular * COST.DC_REGULAR;
    const urcRegularTuition = inputs.urcRegular * COST.URC_REGULAR;
    const dcRetakeTuition = inputs.dcRetake * COST.DC_RETAKE;
    const urcRetakeTuition = inputs.urcRetake * COST.URC_RETAKE;
    const dcImprovementTuition = inputs.dcImprovement * COST.DC_IMPROVEMENT;
    const urcImprovementTuition = inputs.urcImprovement * COST.URC_IMPROVEMENT;
    ...
    const totalFee = COST.SEMESTER_FEE + totalTuitionFee + COST.EXAM_FEE + COST.ESTABLISHMENT_FEE + inputs.previousDues;
    const totalWaiver = totalTuitionFee * (inputs.waiverPercentage / 100);
    ...
    setResults({
      ...
      previousDues: inputs.previousDues,
      totalFee,
      totalWaiver,
      totalPayable,
      installments: [
            ...
            <CardContent>
              <form onSubmit={calculateFees} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" name="dcRegular" value={inputs.dcRegular} onChange={handleInputChange} min="0" />
                </div>
                <div className="space-y-2">
                  <Label>Regular Credit Hours (URC)</Label>
                  <Input type="number" name="urcRegular" value={inputs.urcRegular} onChange={handleInputChange} min="0" />
                </div>
                 <div className="space-y-2">
                  <Label>Retake Credit Hours (DC)</Label>
                  <Input type="number" name="dcRetake" value={inputs.dcRetake} onChange={handleInputChange} min="0" />
                </div>
                <div className="space-y-2">
                  <Label>Retake Credit Hours (URC)</Label>
                  <Input type="number" name="urcRetake" value={inputs.urcRetake} onChange={handleInputChange} min="0" />
                </div>
                <div className="space-y-2">
                  <Label>Improvement Credit Hours (DC)</Label>
                  <Input type="number" name="dcImprovement" value={inputs.dcImprovement} onChange={handleInputChange} min="0" />
                </div>
                 <div className="space-y-2">
                  <Label>Improvement Credit Hours (URC)</Label>
                  <Input type="number" name="urcImprovement" value={inputs.urcImprovement} onChange={handleInputChange} min="0" />
                </div>
                 <div className="space-y-2">
                  <Label>Previous Dues (if any)</Label>
                  <Input type="number" name="previousDues" value={inputs.previousDues} onChange={handleInputChange} min="0" />
                </div>
                <div className="space-y-2">
                  <Label>Waiver on Tuition Fee (%)</Label>
                  <Input type="number" name="waiverPercentage" value={inputs.waiverPercentage} onChange={handleInputChange} min="0" max="100" />
                </div>
                            ...
                            <TableRow>
                                ...
                                <TableCell>
                                    ...
                                    <div className="text-xs text-muted-foreground pl-4">
                                        <p>DC Regular ({inputs.dcRegular} X {COST.DC_REGULAR}) = {currencyFormatter(results.tuitionBreakdown.dcRegular)}</p>
                                        <p>URC Regular ({inputs.urcRegular} X {COST.URC_REGULAR}) = {currencyFormatter(results.tuitionBreakdown.urcRegular)}</p>
                                        <p>DC Repeat ({inputs.dcRetake} X {COST.DC_RETAKE}) = {currencyFormatter(results.tuitionBreakdown.dcRetake)}</p>
                                        <p>URC Repeat ({inputs.urcRetake} X {COST.URC_RETAKE}) = {currencyFormatter(results.tuitionBreakdown.urcRetake)}</p>
                                        <p>DC Improvement ({inputs.dcImprovement} X {COST.DC_IMPROVEMENT}) = {currencyFormatter(results.tuitionBreakdown.dcImprovement)}</p>
                                        <p>URC Improvement ({inputs.urcImprovement} X {COST.URC_IMPROVEMENT}) = {currencyFormatter(results.tuitionBreakdown.urcImprovement)}</p>
                                    </div>
                                </TableCell>
